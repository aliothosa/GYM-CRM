...
class TrainerWorkloadMessageListenerTest {
    ...
    void rejectsAnInvalidActionWithoutCallingBusinessLogic() {
        ...
        assertThrows(InvalidWorkloadException.class, () -> listener(service)
                .onTrainingWorkloadChanged(message("UNKNOWN"), "transaction-123"));
    ...
    void propagatesBusinessFailuresToTheJmsContainerErrorHandler() {
        ...
        assertThrows(InvalidWorkloadException.class, () -> listener(service)
                .onTrainingWorkloadChanged(message("ADD"), "transaction-123"));
    }
    ...
    @Test
    void rejectsBlankIdentityAndNonPositiveDurationWithoutCallingBusinessLogic() {
        TrainerWorkloadService service = mock(TrainerWorkloadService.class);
        Validator validator = Validation.buildDefaultValidatorFactory().getValidator();
        TrainerWorkloadMessageListener listener = new TrainerWorkloadMessageListener(service, validator);
    ...
    @Test
    void installsJmsTransactionIdForProcessingAndCleansItAfterSuccess() {
        TrainerWorkloadService service = mock(TrainerWorkloadService.class);
        TrainerWorkloadMessageListener listener = listener(service);

        doAnswer(invocation -> {
            org.junit.jupiter.api.Assertions.assertEquals("transaction-123", MDC.get("transactionId"));
            return null;
        }).when(service).applyWorkload(any());

        listener.onTrainingWorkloadChanged(message("ADD"), "transaction-123");

        verify(service).applyWorkload(any());
        org.junit.jupiter.api.Assertions.assertNull(MDC.get("transactionId"));
    }

    @Test
    void cleansTransactionIdAfterProcessingFailureAndGeneratesOneWhenMissing() {
        TrainerWorkloadService service = mock(TrainerWorkloadService.class);
        doThrow(new InvalidWorkloadException("failure")).when(service).applyWorkload(any());

        assertThrows(InvalidWorkloadException.class,
                () -> listener(service).onTrainingWorkloadChanged(message("ADD"), null));
